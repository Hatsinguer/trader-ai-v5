...
def render_mesa_day_trade_tab() -> None:
    ...
    # ---------- Barra de mercado rápida ----------
    with st.container():
        st.markdown(
            "<div style='padding:6px 0 4px;font-size:11px;color:gray;'>"
            "⚠️ Dados com possível atraso (Brapi/Yahoo). "
            "Use o campo <b>Preço manual</b> para inserir cotação direta do seu Home Broker."
            "</div>",
    ...
    with col_main:
        ...
        scan_mjolnir = btn_col2.button("🔨 Scan Mjolnir", key="dt_mjolnir")
        btn_col3.caption(f"Atualizado · {datetime.now().strftime('%H:%M:%S')}")
