...
# ============================================================
# RELÓGIOS B3 / NOVA YORK
# ============================================================

def is_b3_open() -> tuple[bool, str]:
    """Verifica se a B3 está aberta agora (horário de Brasília). Retorna (is_open, label)."""
...
def render_market_clocks() -> None:
    ...
    components.html(
        ...
        <style>
            ...
            .ny-open { background: linear-gradient(135deg,#0ea5e9,#0369a1); box-shadow: 0 8px 28px rgba(14,165,233,0.35); }
        ...
        <script>
            ...
            function getB3Status() {
                ...
                const minute = Number(br.minute);
                const tm = hour*60 + minute;
                const wd = br.weekday;
                if(wd==="Sat"||wd==="Sun") return {className:"closed", label:"Mercado fechado — fim de semana"};
                if(tm>=570&&tm<600)  return {className:"preopen", label:"B3: pré-abertura / cancelamento"};
                if(tm>=600&&tm<1015) return {className:"regular", label:"B3: pregão regular aberto ✅"};
                if(tm>=1050&&tm<1125) return {className:"after",  label:"B3: after-market"};
                return {className:"closed", label:"B3: mercado fechado"};
            }
            ...
            function updateClocks() {
                ...
                const nyStatusEl = document.getElementById("ny-status");
                ...
                brCard.classList.add(b3.className);

                if(nyStatusEl) nyStatusEl.textContent = getNYStatus();
            }
